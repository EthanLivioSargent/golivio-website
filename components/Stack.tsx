...
const products: Product[] = [
  ...
  },
  {
    ...
    bullets: [
      "Multi-trade panels (struct + electrical + plumbing)",
      "20 MW → 1+ GW with the same module set",
      "AI-ready: liquid-cooled densities, MV switchgear",
      "Same crews, same playbook, every site",
    ],
  ...
  },
  {
    ...
    bullets: [
      "Land → Grid → Hub → Smart Shell, one pipeline",
  ...
  },
  {
    ...
    bullets: [
      ...
      "PUE / wet-bulb / topology sanity checks",
      "CapEx delta vs. Livio model",
    ...
    <section id="stack" className="section-sky relative isolate scroll-mt-24 overflow-hidden border-b border-line py-20 md:py-28" aria-labelledby="stack-h">
      ...
      <div className="container-page relative">
        <div className="max-w-3xl">
          ...
          <Reveal delay={1}><h2 id="stack-h" className="section-title mt-3 text-balance">Five products. One factory. One prompt.</h2></Reveal>
          <Reveal delay={2}>
        ...
        <div className="mt-12 grid gap-5 lg:grid-cols-2">
          {products.map((p, i) => (
            <Reveal key={p.name} delay={(((i % 4) + 1) as 1 | 2 | 3 | 4)}>
              <a href={p.href} {...(p.href.startsWith("http") ? { target: "_blank", rel: "noopener noreferrer" } : {})} className="group card-light lift relative block h-full overflow-hidden p-6 md:p-8">
                ...
                <div className="relative">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="rounded-full border border-gold/40 bg-gold/15 px-2.5 py-[3px] font-mono text-[10px] font-bold uppercase tracking-[0.16em] text-gold-700">{p.badge}</span>
                    <span className={p.monoDomain ? "font-mono text-[0.8rem] text-text-low" : "text-[0.8rem] text-text-low"}>{p.domain}</span>
                    {p.free && <span className="badge-free ml-auto">● Perpetually free</span>}
                  </div>
                  <h3 className="mt-4 text-[1.4rem] font-bold tracking-[-0.02em] text-text-hi">
                    {p.name}<span className="ml-2 text-[0.92rem] font-semibold text-text-low">— {p.sub}</span>
                  </h3>
                  <p className="mt-3 text-[1.0rem] font-semibold leading-snug text-gold-700">{p.headline}</p>
                  <p className="mt-3 text-[0.95rem] leading-[1.7] text-text-mid">{p.body}</p>
                  <ul className="mt-5 grid gap-2 sm:grid-cols-2">
                    {p.bullets.map((b) => (
                      <li key={b} className="flex items-start gap-2 text-[0.88rem] text-text-mid">
                        <span aria-hidden className="mt-[7px] inline-block h-1 w-1 rounded-full bg-gold-600" />
                        <span>{b}</span>
                      </li>
                    ))}
                  </ul>
                  <div className="mt-6 inline-flex items-center gap-1.5 text-[0.92rem] font-semibold text-gold-700 transition-transform group-hover:translate-x-[2px]">
