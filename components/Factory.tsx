    ...
    <section id="factory" className="section-navy relative isolate scroll-mt-24 overflow-hidden border-b border-navy-900 py-20 md:py-28" aria-labelledby="factory-h">
      ...
      <div className="container-page relative">
        <div className="max-w-3xl">
          ...
          <Reveal delay={1}><h2 id="factory-h" className="section-title mt-4 text-balance text-invert-hi">A buildable AI data center, shipped in panels.</h2></Reveal>
          <Reveal delay={2}>
          ...
          {features.map((f, i) => (
            <Reveal key={f.title} delay={(((i % 3) + 1) as 1 | 2 | 3)}>
              <article className="card-dark lift-dark h-full p-7 md:p-8">
                ...
                <h3 className="text-[20px] font-bold tracking-[-0.01em] text-invert-hi">{f.title}</h3>
                <p className="mt-2 text-[16px] leading-[1.65] text-invert-low">{f.body}</p>
              </article>
            </Reveal>
          ))}
        </div>
